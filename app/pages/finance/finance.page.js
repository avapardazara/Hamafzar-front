...
export function useFinancePage() {
  ...
  const upcoming7 = computed(() =>
    Array.isArray(state.upcoming_7) ? state.upcoming_7 : []
  );
  ...
  const coursesSummary = computed(() => {
    const rows = Array.isArray(state.courses) ? state.courses : [];

    return rows.map((r) => {
      const nominal = Number(r.nominal_total ?? r.face ?? r.fee ?? 0);
      const received = Number(r.received_total ?? r.received ?? r.paid ?? 0);
      const receivable =
        r.receivable_total != null
          ? Number(r.receivable_total)
          : Math.max(nominal - received, 0);

      const mentorShareTotal = Number(
        r.mentor_share_total ?? r.mentor_share ?? 0
      );
      const mentorPaidTotal = Number(r.mentor_paid_total ?? r.mentor_paid ?? 0);
      const mentorDueTotal =
        r.mentor_due_total != null
          ? Number(r.mentor_due_total)
          : Math.max(mentorShareTotal - mentorPaidTotal, 0);

      return {
        // مقادیر خام API
        ...r,

        // برای سازگاری با تمپلیت فعلی تب «دوره‌ها»
        ...
        paid: r.paid ?? received,

        remain: r.remain ?? receivable,

        mentor_share: r.mentor_share ?? mentorShareTotal,
        mentor_paid: r.mentor_paid ?? mentorPaidTotal,
        mentor_due: r.mentor_due ?? mentorDueTotal,
      };
    });
  });

  const mentorsSummary = computed(() => {
    const rows = Array.isArray(state.mentors) ? state.mentors : [];

    return rows.map((r) => {
      const share = Number(r.share ?? 0);
      const paid = Number(r.paid ?? 0);
      const due = r.due != null ? Number(r.due) : Math.max(share - paid, 0);

      return {
        ...r,
        courses: r.courses ?? r.courses_count ?? 0,
        received: Number(r.received ?? share),
        share,
        paid,
        due,
      };
  ...
  const installments = computed(() => {
    const rows = Array.isArray(state.installments) ? state.installments : [];

    return rows.map((p) => {
      const amount = Number(p.amount ?? p.amount_total ?? p.remain ?? 0);

      return {
        ...p,
        amount,
        amount_total: p.amount_total ?? amount,
        // اگر backend عنوان نداد، یک عنوان پیش‌فرض انسانی بساز
        title: p.title || p.plan_title || (p.seq ? `قسط ${p.seq}` : "قسط"),
  ...
  const expenseKpis = computed(() => ({
    mtd: Number(
      (state.kpis && (state.kpis.mtd_expense || state.kpis.expense_mtd)) || 0
    ),
    total: Number(
